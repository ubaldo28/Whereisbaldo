  ...
  try {
    ...
    const emailRes = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        reply_to: `${name} <${email}>`,
        subject: topic ? `[${topic}] New message from ${name}` : `New message from ${name}`,
        html: `
          <div style="font-family: Georgia, serif; max-width: 600px; margin: 0 auto; padding: 40px 20px; color: #1a1a1a;">
            <h2 style="color: #c49959; margin-bottom: 24px;">New Message from whereisbaldo.com</h2>
            <p><strong>Name:</strong> ${name}</p>
            <p><strong>Email:</strong> ${email}</p>
            <a href="mailto:${email}?subject=${encodeURIComponent('Re: your message to whereisbaldo.com')}" style="display:inline-block;margin:16px 0;padding:12px 24px;background:#c49959;color:#fff;text-decoration:none;border-radius:6px;font-weight:bold;">Reply to ${name} &rarr;</a>
